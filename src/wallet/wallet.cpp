...
void CWallet::BestStakeSeen(uint256& hash)
{
...
bool CWallet::CreateCoinStake(const CWallet& wallet, unsigned int nBits, CMutableTransaction& txNew, unsigned int& nTxNewTime)
{
    txNew.vin.clear();
    txNew.vout.clear();
    ...
    for(const std::pair<const CWalletTx*, unsigned int> &pcoin : setStakeCoins)
    {
        ...
        {
          nTryTime = nTxNewTime + nHashDrift - i;
          bool hashFound = CheckStakeKernelHash(nBits, block, pcoin.first->tx, prevoutStake, nTryTime, nHashDrift, false, hashProofOfStake);
