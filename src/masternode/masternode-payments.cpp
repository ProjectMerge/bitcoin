...
{
    bool isProofOfStakeBlock = block.IsProofOfStake();
    bool isBlockRewardValueMet = (block.vtx[isProofOfStakeBlock]->GetValueOut() <= blockReward);

    LogPrint(BCLog::MNPAYMENTS, "block.vtx[%d]->GetValueOut() %lld <= blockReward %lld\n", isProofOfStakeBlock, block.vtx[isProofOfStakeBlock]->GetValueOut(), blockReward);
