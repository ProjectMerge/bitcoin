...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_16_CLIENT_COMPAT_MODE,            4070908800ULL), // OFF
...
void CSporkManager::ProcessSpork(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        auto m_chain = ::ChainActive();
        uint256 hash = spork.GetHash();

        std::string strLogMsg;
        {
            LOCK(cs_main);
            EraseInvRequest(pfrom->GetId(), CInv(MSG_SPORK, hash));
            if(!m_chain.Tip()) return;
            strLogMsg = strprintf("SPORK -- hash: %s id: %d value: %10d bestHeight: %d peer=%d", hash.ToString(), spork.nSporkID, spork.nValue, m_chain.Height(), pfrom->GetId());
        }
